...
with a (city, tot) as (select city,sum(amount) from acceptor group by city),d (city,tot) as(select city,sum(amount) from donar group by city),res(a,b) as (select a.city,round(d.tot/a.tot,4)from a join d on a.city = d.city)select a as city,b as ratio from res where b=(select max(b) from res) or b=(select min(b) from res) ; 
